...
Even if this feels a bit unsophisticated, this is not awful, 
...
The weight of male basset hounds follows a $\Ncal(65, 10)$
distribution while females have weight distributed as 
$\Ncal(55, 10)$. The weight of a randomly selected basset
can be sampled as
...
Estimation of $\Pcal_0, \Pcal_1$ may be more difficult. If the 
...
For now, let us assume $\Pcal_{0}, \Pcal_1$ have been estimated
...
$\Pcal_0, \Pcal_1$. Let us review these in ascending order of
complexity. 
...
In Naive Bayes, we assume $\Pcal_0, \Pcal_1$ are multivariate normal
...
$$\hat{\mu}_0 = \frac{1}{|\Ccal_0|} \sum_{i \in \Ccal_0} \bx_i$$

where $\Ccal_0$ is the set of observations from class 0 and 

$$\hat{\mu}_1 = \frac{1}{|\Ccal_1|} \sum_{i \in \Ccal_1} \bx_i.$$
The variance estimates are a bit trickier: 

$$\hat{\Sigma}_{ii} = \frac{1}{n}\left(\sum_{j\in\Ccal_0} (x_{ji} - \hat{\mu}_{0i})^2 + \sum_{j\in\Ccal_1} (x_{ji} - \hat{\mu}_{1i})^2\right)$$
