...
2. True/False: The singular values of a (real-valued) matrix are always positive. 
   ...
   **Note:** The intended answer to this question is 
   `True`, but I will allow `False` because I did not
   specify the form of the SVD used. For certain 
   SVD algorithms, especially when applied to rank
   deficient matrices, a singular value of zero is
   produced; for other algorithms, the decomposition
   stops before producing zeros.
...
7. Select One: Which of the following pairs of vectors are
   ...
   **Note:** As presented, the question was missing 
   an important `not`. As such, I am accepting any
   answer for this question and all students are
   receiving full credit. The important point,
   however, is that loading vectors are mutually
   orthogonal, score vectors are mutually orthogonal, 
   and there isn't an orthogonality relationship
   between loading and score vectors because they are
