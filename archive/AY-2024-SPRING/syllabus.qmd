...
- Once work is submitted and graded by the instructor, you cannot use late days
  to re-submit a revised copy. The instructor may grade work quickly after the
  standard submission deadline passes, so make sure to only submit 'final' 
  (or 'final enough') versions if using late days. 
- Late days cannot be used to submit work after the registrar mandated 
  last day of the semester. (This date may be found on Baruch's academic
  calendar.) In particular, the end-of-semester date sets the final day on
  which the course final report may be submitted (even if you have sufficient
  remaining late days to submit beyond that window) without explicit instructor
  pre-authorization (see above).
